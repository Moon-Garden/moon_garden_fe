...
RSpec.describe 'garden be service' do
  ...
  context 'create_garden' do
    it 'passes in garden info and creates a garden associated with a user', :vcr do
      ...
      GardenFacade.destroy_garden(@user_response[:data][:id], @response[:data][:id])
    end
  end
  ...
  context 'destroy_garden' do
    it 'destroys a users garden', :vcr do
      ...
      user_gardens_length1 = GardenFacade.get_gardens(@user_response[:data][:id]).length

      GardenService.destroy_garden(@user_response[:data][:id], garden.id)

      user_gardens_length2 = GardenFacade.get_gardens(@user_response[:data][:id]).length
